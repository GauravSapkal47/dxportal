...
export class InternalportaldashboardComponent implements OnInit {
  ...
  public barChartData: ChartDataSets[] = [{ data: [23, 6, 10, 15, 20, 25] }];
    ...
    this.activatedRoute.queryParams.subscribe((params) => {
      ...
      if (productid) {
        ...
        if (
          ...
          for (var a = 0; a < this.invoicestatusList.length; a++) {
            if (this.status == this.invoicestatusList[a].value) {
              ...
              this.searchList.controls.Status.setValue(
                this.invoicestatusList[a].stype
              );
            }
          }
          console.log("this.status " + this.status);
        ...
        if (
          ...
          this.searchList.controls.fromDate.setValue(
            moment(new Date(this.fromdateofduration))
              .format("DD/MM/YYYY")
              .trim()
          );
        ...
        if (
          ...
          this.todateofduration != null &&
          this.todateofduration != "null"
        ) {
          console.log("this.todateofduration =>" + this.todateofduration);
          this.searchList.controls.toDate.setValue(
            moment(new Date(this.todateofduration)).format("DD/MM/YYYY")
          );
        }
            ...
            .subscribe((res) => {
              ...
              for (var b = 0; b < this.tempPlantList1.length; b++) {
                if (this.plant == this.tempPlantList1[b].PLANTCODE) {
                  this.searchList.controls.Plant.setValue(
                    this.tempPlantList1[b].PLANTNAME
                  );
                }
              }
            });
        ...
        if (
          this.invamountfrom != "NA" &&
          this.invamountfrom != "" &&
          this.invamountfrom != null &&
          this.invamountfrom != "null"
        ) {
        ...
        if (
          this.ageingfrom != "NA" &&
          this.ageingfrom != "" &&
          this.ageingfrom != null &&
          this.ageingfrom != "null"
        ) {
          this.ageingto = spliteddetails[11];
          if (this.ageingfrom != "30") {
            var ageinggap =
              "From " + this.ageingfrom + " to " + this.ageingto + " days";
          } else {
        ...
        this.internalportalservice
          ...
          .subscribe((res) => {
            ...
            if (res[0].invoiceCountAsPerStatus) {
              ...
              if (
                this.invoiceCountAsPerStatus?.P == null &&
                this.invoiceCountAsPerStatus?.M != null
              ) {
                ...
                this.pendingPo =
                  Number(this.invoiceCountAsPerStatus.P) +
                  Number(this.invoiceCountAsPerStatus.M);
              }
            }
            if (res[0].message == "Success") {
              ...
              this.endpagenumber = Math.ceil(
                Number(res[0].invoicedetailsrecords) / this.pageSize
              );
              if (
                Number(res[0].invoicedetailsrecords) /
                  this.invoicepopaginationnumber <=
                this.pageSize
              ) {
                  ...
                  if (this.statusList.length != 0) {
                    for (var j = 0; j < this.statusList.length; j++) {
                      if (
                        ...
                        this.statusList[j]["INVOICENUMBER"] ==
                          this.invoicedetailsList[k]["INVOICENUMBER"] &&
                        this.statusList[j]["PONUMBER"] ==
                          this.invoicedetailsList[k]["PONUMBER"]
                      ) {
                        console.log(
                          "insise A--------------------------==============",
                          this.invoicedetailsList[k]["INVOICENUMBER"]
  ...
  multiPOData(invoicenumber, ponumber, multiponumber) {
    localStorage.removeItem("options");
    ...
    if (multiponumber == undefined) {
      multiponumber = ponumber;
      this.router.navigate(["./internaltrackinvoicedetails"], {
        queryParams: {
          order: btoa(invoicenumber),
          po: btoa(ponumber),
          origin: btoa("internaldashboard"),
        },
      });
    } else {
      this.router.navigate(["./Internalmultitrackinvoicedetails"], {
        queryParams: {
          order: btoa(invoicenumber),
          po: btoa(ponumber),
          mpo: btoa(multiponumber),
          origin: btoa("internaldashboard"),
        },
      });
    }
  ...
  callDynamicData() {
    if (
      this.searchList.value.Requisitioner != null &&
      this.searchList.value.Requisitioner != undefined &&
  ...
  getColleaguesData(searchString) {
    ...
    this.internalportalservice
      .searchPeople(searchString, true, "NA", "NA")
      .subscribe(
        (res) => {
  ...
  addColleaguesToSuggestionList(e, item) {
    ...
    if (this.selectedColleaguesList.length > 0) {
      ...
      this.selectedColleaguesList.push(item);
      console.log(
        "selectedColleaguesList===============>",
        this.selectedColleaguesList
      );
      //to not delete if item is newly added.
      if (this.colleaguesDataList.indexOf(item) >= 0)
        this.colleaguesDataList.splice(
          this.colleaguesDataList.indexOf(item),
          1
        );
  ...
  removeColleaguesFromSuggestionList(e, item) {
    ...
    this.selectedColleaguesList.splice(
      this.selectedColleaguesList.indexOf(item),
      1
    );
  }
  ...
  SearchData() {
    ...
    // return;
    console.log(
      "this.searchList.value.modeType " + this.searchList.value.modeType
    );
    ...
    if (this.searchList.value.modeType == "INVOICE") {
      ...
      if (this.searchList.controls["fromDate"].value) {
        ...
        this.fromdateofduration = moment(
          new Date(this.searchList.controls["fromDate"].value)
        )
          .format("DD/MM/YYYY")
          .trim();
        this.todateofduration = moment(
          new Date(this.searchList.controls["toDate"].value)
        )
          .format("DD/MM/YYYY")
          .trim();
      } else {
      ...
      console.log(
        "this.searchList.value.Ageing " + this.searchList.value.Ageing
      );

      if (
        this.searchList.value.Ageing != undefined &&
        this.searchList.value.Ageing != "ALL"
      ) {
      ...
      console.log(
        "this.searchList.value.ageingfrom " + this.searchList.value.ageingfrom
      );
      ...
      this.internalportalservice
        ...
        .subscribe((res) => {
          ...
          if (res[0].message == "Success") {
            ...
            if (
              Number(res[0].popages) / this.invoicepopaginationnumber <=
              this.pageSize
            ) {
              this.disablenext = true;
            } else {
  ...
  getPurchaseGroupData(e) {
    ...
    this.internalportalservice
      .getPurchaseGroup(e.target.value.toUpperCase())
      .subscribe((res) => {
  ...
  sortdata(status) {
    ...
    this.internalportalservice
      .getinvoicedetailsbasedonemailid(
        this.emailid,
        this.internalpaginationpagenumber,
        this.currstatus,
        "NA",
        "NA",
        "NA",
  ...
  getVendorData(e) {
    this.vendorList = [];
    this.internalportalservice
      .getVendorData(e.target.value.toUpperCase())
      .subscribe((res) => {
  ...
  getPurchaseData(e) {
    this.purchaseList = [];
    this.internalportalservice
      .getpurchasegroupData(e.target.value.toUpperCase())
      .subscribe((res) => {
  ...
  ValidateEmail(event) {
    ...
    if (
      /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,9})+$/.test(event.target.value)
    ) {
      // [<>=]{1,2}
      this.submitted = false;
  ...
  downloadpolist() {
    ...
    this.purchaseOrderListService
      .getinternalpolistdashboardfile(
        this.povalue,
        this.status,
        this.fromdateofduration,
        this.todateofduration,
  ...
  download(savedfilename, singlepodownload, actualfilename) {
    ...
    this.trackOrderListService
      .getinternalfile(savedfilename, singlepodownload)
      .subscribe(
        (result) => {
          console.log(result);
          console.log("filename===>" + result[0].filename);
  ...
  Validateamount(amount) {
    if (this.searchList.value.modeType == "INVOICE") {
      if (
        (this.searchList.value.invoicemin == "" &&
          this.searchList.value.invoicemax != "") ||
        (this.searchList.value.invoicemin != "" &&
          this.searchList.value.invoicemax == "") ||
        this.searchList.value.invoicemin.includes("-") ||
        this.searchList.value.invoicemax.includes("-") ||
        ...
        Number(this.searchList.value.invoicemin) >
        Number(this.searchList.value.invoicemax)
      ) {
        this.disable = true;
        this.error = true;
        this.subceed = false;
      ...
      if (
        (this.searchList.value.pomin == "" &&
          this.searchList.value.pomax != "") ||
        (this.searchList.value.pomin != "" &&
          this.searchList.value.pomax == "") ||
        this.searchList.value.pomin.includes("-") ||
        this.searchList.value.pomax.includes("-") ||
        ...
        Number(this.searchList.value.pomin) >
        Number(this.searchList.value.pomax)
      ) {
        this.disable = true;
        this.error = true;
        this.subceed = false;
