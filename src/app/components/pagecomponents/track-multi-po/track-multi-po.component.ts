...
@Component({
  selector: 'app-track-multi-po',
  templateUrl: './track-multi-po.component.html',
  styleUrls: ['./track-multi-po.component.css']
})
export class TrackMultiPoComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, private router: Router, private route: ActivatedRoute, private purchaseOrderListService: PurchaseOrderListService,
    ...
    multiPOData(invoicenumber, ponumber, creditnote, multiponumber) {
      ...
      this.router.navigate(['./trackInvoiceDetails'], { queryParams: { order: btoa(invoicenumber), po: btoa(ponumber), credit: btoa(creditnote), mpo: btoa(multiponumber)  } });
    }
    downloadinvoicelist() {
      this.invoicenumber = [];
      this.ponumber = [];
