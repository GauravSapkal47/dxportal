...
export class TrackInvoiceListComponent implements OnInit {
  ...
  ResetData() {
    ...
    // this.invoiceList.controls.duration.setValue("");
  ...
  multiPOData(invoicenumber, ponumber, creditnote, multiponumber) {
    ...
    {
      this.router.navigate(['./trackMultiPODetails'], { queryParams: { order: btoa(invoicenumber), po: btoa(ponumber), credit: btoa(creditnote), mpo: btoa(multiponumber)  } });
    }
  }
...
this.trackOrderListService.downloadEncryptedFile(data,this.extensionType);
  ...
  resubmitInvoice(invNo, poNo, vendorid, type,mpo,planttype) {
