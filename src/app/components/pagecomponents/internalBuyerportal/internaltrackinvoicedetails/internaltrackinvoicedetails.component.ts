...
export class InternaltrackinvoicedetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      ...
      this.slice_span_height = document.getElementById("slice_span");
      console.log("this.slice_span_height > ", this.slice_span_height.clientHeight);
      console.log("this.slice_count > ", this.slice_count);

      // while (this.slice_span_height > 25) {
      //   this.slice_count--;
      //   console.log("this.slice_count =>", this.slice_count);
      // }
    });
    this.invoicedata = [];
    this.internalportalservice
      .getlistitemsforinvoicenumbers(this.data, this.ponumber)
      .subscribe((res) => {
        ...
        if (res[0].message == "Success") {
          ...
          for (let i = 0; i < this.invoicedata.length; i++) {
            if (this.invoicedata[i].ACCEPTEDQTY == null) {
              this.invoicedata[i].ACCEPTEDQTY = 0.0;
            }
            if (this.invoicedata[i].GRNNUMBER != null) {
              this.invoicedata[i].TOTALCALCULATEAMOUNT = Number(
                parseFloat(this.invoicedata[i].ACCEPTEDQTY) *
                  parseFloat(this.invoicedata[i].RATEPERQTY)
              );
            }
            if (this.invoicedata[i].GRNNUMBER == null) {
              this.invoicedata[i].TOTALCALCULATEAMOUNT = Number(
                parseFloat(this.invoicedata[i].QUANTITY) *
                  parseFloat(this.invoicedata[i].RATEPERQTY)
              );
            }
          }
          ...
          this.paidamount = Number(this.invoicedata[0].PAYMENTAMOUNT)
            .toFixed(2)
            .replace(/\B(?=(?:(\d\d)+(\d)(?!\d))+(?!\d))/g, ",");
          this.user = sessionStorage.getItem("Requisitioner");
          this.totalamount = Number(this.invoicedata[0].TOTALAMOUNT)
            .toFixed(2)
            .replace(/\B(?=(?:(\d\d)+(\d)(?!\d))+(?!\d))/g, ",");
          ...
          if (this.invoicedata[0].MULTIACTUALFILENAME != null) {
            ...
            console.log(
              "this.multipleactualfilename" + this.multipleactualfilename
            );
            this.multiplesavedfilename = this.invoicedata[0].MULTISAVEDFILENAME;
            console.log(
              "this.multiplesavedfilename " + this.multiplesavedfilename
            );
          ...
          if (this.invoicedata[0].ENDUSERSUPPACTUALFILE != null) {
            this.multipleactualendfilename =
              this.invoicedata[0].ENDUSERSUPPACTUALFILE;
            console.log(
              "this.multipleactualfilename" + this.multipleactualfilename
            );
            this.multiplesavedendfilename =
              this.invoicedata[0].ENDUSERSUPPSAVEDFILE;
            console.log(
              "this.multiplesavedfilename " + this.multiplesavedendfilename
            );
          ...
          this.internalportalservice
            .trackinvoicestatusforinternal(this.invoicenumber, this.ponumber)
            .subscribe((res) => {
              if (res[0].message == "Success") {
                ...
                for (var z = 0; z < this.trackinvoicelistdata.length; z++) {
                  ...
                  if (this.trackinvoicelistdata[z].PENDINGSTATUS) {
                    this.pending = true;
                    this.pendingstatus =
                      this.trackinvoicelistdata[z].PENDINGSTATUS;
                    this.pendingtime =
                      this.trackinvoicelistdata[z].PENDINGMODIFIEDTIME;
                  } else if (this.trackinvoicelistdata[z].RESUBMITSTATUS) {
                    console.log("inini");
                    this.resubmit = true;
                    this.resubmitstatus =
                      this.trackinvoicelistdata[z].RESUBMITSTATUS;
                    this.resubmittime =
                      this.trackinvoicelistdata[z].RESUBMITMODIFIEDTIME;
                    this.previoustoresubmitinvoicenumber =
                      this.trackinvoicelistdata[z].RESUBMITTEDINVOICENO;
                  } else if (this.trackinvoicelistdata[z].RETURNSTATUS) {
                    console.log("inini");
                    this.return = true;
                    this.returnstatus =
                      this.trackinvoicelistdata[z].RETURNSTATUS;
                    this.returntime =
                      this.trackinvoicelistdata[z].RETURNMODIFIEDTIME;
                  } else if (this.trackinvoicelistdata[z].MPENDINGSTATUS) {
                    console.log("inini");
                    this.mpending = true;
                    this.mpendingstatus =
                      this.trackinvoicelistdata[z].MPENDINGSTATUS;
                    this.mpendingtime =
                      this.trackinvoicelistdata[z].MPENDINGMODIFIEDTIME;
                  } else if (this.trackinvoicelistdata[z].ACCEPTEDSTATUS) {
                    this.accepted = true;
                    this.acceptedstatus =
                      this.trackinvoicelistdata[z].ACCEPTEDSTATUS;
                    this.acceptedtime =
                      this.trackinvoicelistdata[z].ACCEPTEDMODIFIEDTIME;
                  } else if (this.trackinvoicelistdata[z].REJECTEDSTATUS) {
                    this.rejected = true;
                    this.rejectedstatus =
                      this.trackinvoicelistdata[z].REJECTEDSTATUS;
                    this.rejectedtime =
                      this.trackinvoicelistdata[z].REJECTEDMODIFIEDTIME;
                  } else if (this.trackinvoicelistdata[z].PROCESSEDSTATUS) {
                    this.processed = true;
                    this.processedstatus =
                      this.trackinvoicelistdata[z].PROCESSEDSTATUS;
                    this.processedtime =
                      this.trackinvoicelistdata[z].PROCESSEDMODIFIEDTIME;
                  } else if (this.trackinvoicelistdata[z].PARTIALLYPAIDSTATUS) {
                    this.partialpaid = true;
                    this.partialpaidstatus =
                      this.trackinvoicelistdata[z].PARTIALLYPAIDSTATUS;
                    this.partialpaidtime =
                      this.trackinvoicelistdata[z].PARTIALLYPAIDMODIFIEDTIME;
                  } else if (this.trackinvoicelistdata[z].FULLYPAIDSTATUS) {
                    this.paid = true;
                    this.paidstatus =
                      this.trackinvoicelistdata[z].FULLYPAIDSTATUS;
                    this.paidtime =
                      this.trackinvoicelistdata[z].FULLYPAIDSTATUSMODIFIEDTIME;
                  } else if (this.trackinvoicelistdata[z].REVERSEDSAPSTATUS) {
                    this.reversed = true;
                    this.reversedstatus =
                      this.trackinvoicelistdata[z].REVERSEDSAPSTATUS;
                    this.reversedtime =
                      this.trackinvoicelistdata[z].REVERSEDSAPMODIFIEDTIME;
                  }
                }
              }
            });
  ...
  getstatuschatmessages(invnum, ponum) {
    this.internalportalservice
      ...
      .subscribe((res) => {
        ...
        console.log(
          this.statuschatmessage,
          "==============customManagerListNames=============="
        );
      });
  }
  // downloadfile() {
  ...
  downloadfile(savedfilename, actualfilename, type) {
    ...
    this.loaderservice.show();
    this.trackOrderListService.getfile(savedfilename, type).subscribe(
      (result) => {
        console.log(result);
          ...
          if (window.navigator && window.navigator.msSaveOrOpenBlob) {
            ...
            if (type == "listdownload") {
              ...
              window.navigator.msSaveOrOpenBlob(
                blob,
                "Supporting_Invoice_Document.zip"
              );
            }
          } else if (agent.indexOf("firefox") > -1) {
            var byteCharacters = atob(data);
            ...
            if (type == "listdownload") {
              ...
              link.download = filename;
              // link.click();
              var extensionName = filename.substr(
    ...
    this.internalportalservice
      ...
      .subscribe((res) => {
        if (res[0].invoiceData.length > 0) {
          ...
          // if EUmanager Null
          this.endUserEmpty = this.customManagerList[0].ENDUSERSTATUS;
          if (this.customManagerList[0].EUMANAGER != null) {
            console.log(
              "EUMANAGER=========>",
              this.customManagerList[0].ENDUSEID,
              this.customManagerList[0].ENDUSERSTATUS
            );
            // this.endUserEmpty=this.customManagerList[0].ENDUSERSTATUS;
          ...
          console.log(
            ...
            "==============getMultipleManagerList=============="
          );
          for (
            let loop = 0;
            loop <= this.customManagerList.length - 1;
            loop++
          ) {
            if (this.customManagerList[loop].EUMANAGER != null) {
              var name =
                this.customManagerList[loop].EUMANAGER.match(/^([^.]*)./)[1];
  ...
  getColleaguesData(searchString) {
    ...
    this.internalportalservice
      .searchPeople(searchString, true.valueOf, "NA", "NA")
      .subscribe(
        (res) => {
