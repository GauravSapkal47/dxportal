...
export class DashboardComponent implements OnInit {
  ...
  constructor(private router: Router, private purchaseOrderListService: PurchaseOrderListService,
    private TrackOrderListService: TrackOrderListService, private trackOrderListService: TrackOrderListService,
  ...
  getPOmessageStatus() {
    this.purchaseOrderListService.getReadStatus().subscribe(res => { console.log("response", res[0].poQueryList)
...
this.uniqueInvNumber = invNumber;
  ...
  getPoDetails(POnumber, Date, Amount, Query, Status) {
    ...
    this.purchaseOrderListService.getPODetails(POnumber).subscribe(res => {
      ...
      this.router.navigate(['/poOrderView'], { queryParams: { POnumber: btoa(POnumber), Date: btoa(Date), Amount: btoa(Amount),
