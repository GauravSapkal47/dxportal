...
import {
  createDeliveryModel,
  createDeliverySubmissionsModel,
  createDeliveryInvoiceSubmissionsModel,
  deliveryDetails,
  orderdetailsModel,
  LineItemOrderModel,
...
export class PurchaseOrdersListComponent implements OnInit {
  ...
  get fromDate() {
    return this.POOrderList.get("fromDate").value;
  }
  get toDate() {
    return this.POOrderList.get("toDate").value;
  }
      ...
      .subscribe(
        (res) => {
          if (res[0].message == "Success") {
            ...
            console.log(
              "powodeloveriees ANgaj ===========>",
              this.powithnodeliveries
            );

            this.poDataList = res[0].poData;
            for (let i = 0; i < this.poDataList.length; i++) {
              this.poDataList[i].AMOUNT = Number(this.poDataList[i].AMOUNT)
                .toFixed(2)
            ...
            this.endpagenumber = Math.ceil(
              Number(res[0].popages) / this.pageSize
            );
            if (
              Number(res[0].popages) / this.popaginationnumber <=
              this.pageSize
            ) {
              ...
              for (var k = 0; k < this.poDataList.length; k++) {
                ...
                if (this.statusList.length != 0) {
                  for (var j = 0; j < this.statusList.length; j++) {
                    if (
                      this.statusList[j]["STATUS"] == "A" &&
                      this.statusList[j]["PONUMBER"] ==
                        this.poDataList[k]["PO_NUMBER"]
                    ) {
            ...
            this.purchaseOrderListService
              ...
              .subscribe((res1) => {
                ...
                setTimeout(() => {
                  ...
                  for (var k = 0; k < this.poDataList.length; k++) {
                    console.log(
                      "this.statusList.length " + this.statusList.length
                    );
                    if (this.statusList.length != 0) {
                      for (var j = 0; j < this.statusList.length; j++) {
                        if (
            ...
            this.purchaseOrderListService
              ...
              .subscribe((res1) => {
                if (res1[0].message != "No Data Found for given Vendor Id") {
                  ...
                  for (let i = 0; i < this.poDataList.length; i++) {
                    this.poDataList[i].AMOUNT = Number(
                      this.poDataList[i].AMOUNT
                    )
                      .toFixed(2)
                      .replace(/\B(?=(?:(\d\d)+(\d)(?!\d))+(?!\d))/g, ",");
                  }
      ...
      for (var k = 0; k < this.poDataList.length; k++) {
        for (var j = 0; j < this.statusList.length; j++) {
          if (this.statusList.length != 0) {
            if (
              this.statusList[j]["STATUS"] == "A" &&
              this.statusList[j]["PONUMBER"] == this.poDataList[k]["PO_NUMBER"]
            ) {
              $("#green" + k).addClass("displayBlock");
              $("#white" + k).addClass("displayNone");
            } else {
  ...
  formatDate(date) {
    var dt = new Date(date),
      ...
      //   d.getMinutes(),
      //   d.getSeconds()].join(':') +'.0';
      dformat = `${dt.getFullYear().toString().padStart(4, "0")}-${(
        dt.getMonth() + 1
      )
        .toString()
  ...
  sendConfirmation(status, message, subject, topic) {
    ...
    if (
      (message == undefined && subject != "SubmitQuery") ||
      (message == "" && subject != "SubmitQuery")
    ) {
      message = "PO Accepted";
    } else if (subject != "SubmitQuery") {
      message = "PO Rejected -" + message;
    ...
    this.purchaseOrderListService
      .submitConfirmation(
        this.purchaseOrderListModel.PONumber,
        status,
        message,
        subject,
        topic
    ...
    this.purchaseOrderListService.getPODetails(POnumber).subscribe((res) => {
      if (res[0].poData && !res[0].message) {
        ...
        for (let i = 0; i < this.uniquelineitems.length; i++) {
          this.uniquelineitems[i].RATEperqty =
            this.uniquelineitems[i].RATEPERQTY;
          this.uniquelineitems[i].RATEPERQTY = Number(
            this.uniquelineitems[i].RATEPERQTY
          )
            .toFixed(2)
            .replace(/\B(?=(?:(\d\d)+(\d)(?!\d))+(?!\d))/g, ",");
        ...
        console.log(
          "this.uniquelineitems======>>>",
          JSON.stringify(this.uniquelineitems)
        );
        ...
        console.log(
          "Set items this.lineitemnumberset ==>" + this.lineitemnumberset
        );
        ...
        this.poDetail = res[0].poData.sort(
          (a, b) => Number(a.LINEITEMNUMBER) - Number(b.LINEITEMNUMBER)
        );
        // // this.poDetail = res[0].poData
        ...
        this.dialogBox.popUpOpen2(
          "PO line item details not present",
          "error",
          "puchaseorderlist"
        );
        this.loaderservice.hide();
  ...
  openAccordionTab(event) {
    ...
    // $('.atbl-container').removeClass('active');  //close all nested accordion
    if (!event.target.classList.contains("active")) {
      $(".customtbl-accordion-head.active + #" + id).removeClass(
        "customtblHide"
      );
    } else {
      $(".customtbl-accordion-head.active + #" + id).show();
  ...
  openAccordionTab1(event) {
    ...
    console.log(id);
    $(".tbl-accordion-content").hide();
    $(".customtbl-accordion-head-delivery.active + #" + id).addClass(
      "customtblHide"
    );
    // $('.atbl-container').removeClass('active');  //close all nested accordion
    if (!event.target.classList.contains("active")) {
      $(".customtbl-accordion-head-delivery.active + #" + id).removeClass(
        "customtblHide"
      );
    } else {
  ...
  openAccordioninnerTab(event, i, j) {
    ...
    $(".customtbl-accordion-head-inner .plusminusArrow").removeClass(
      "fa-minus"
    );
    ...
    if ($("#lineitem" + i + j).hasClass("active")) {
      ...
      $(".customtbl-accordion-head-inner.active .plusminusArrow").addClass(
        "fa-minus"
      );
      $(".customtbl-accordion-head-inner.active .plusminusArrow").removeClass(
        "fa-plus"
      );
    }
  ...
  openAccordionNestedTab(event, lineitemnumber, ponumber) {
    ...
    for (var k = 0; k < this.poDetail.length; k++) {
      // }
      if (
        this.poDetail[k].ORDERNUMBER != null &&
    ...
    if (clickthrough == "orderitem" || clickthrough == "lineitem") {
      ...
      if (this.invoicenumber == undefined) {
        this.router.navigate(["./submitInvoiceFromList"], {
          queryParams: {
            order: btoa(clickthrough),
            vendorId: btoa(vendorid),
            contactpersonemailid: btoa(contactpersonemail),
            dt: btoa(date),
          },
        });
      } else {
        this.router.navigate(["./submitInvoiceFromList"], {
          queryParams: {
            ...
            contactpersonemailid: btoa(contactpersonemail),
            dt: btoa(date),
            in: btoa(this.invoicenumber),
            id: btoa(this.invoicedate),
          },
        });
      }
  ...
  downloadPo(po_number, singlepodownload) {
    ...
    this.DecodedFile = [];
    this.trackOrderListService.getfile(po_number, singlepodownload).subscribe(
      (result) => {
        console.log(result);
        ...
        if (this.DecodedFile) {
          ...
          if (window.navigator && window.navigator.msSaveOrOpenBlob) {
            ...
            var extensionName = filename.substr(filename.lastIndexOf(".") + 1);
            this.getFileType(extensionName);
            this.trackOrderListService.downloadEncryptedFile(
              data,
              this.extensionType
            );
          }
  ...
  ) // orderDetails(orderdetails)
  {
    ...
    this.router.navigate(["/poOrderDispatched"], {
      queryParams: {
        orderNo: orderNo,
        Date: Date,
        Quantity: Quantity,
  ...
  searchorders() {
    ...
    this.disableprevious = true;
    console.log(
      "this.POOrderList.controls['PONumber'].value ==>" +
        this.POOrderList.controls["duration"].value
    );
    console.log(
      "this.POOrderList.controls['fromDate'].value ==>" +
        this.POOrderList.controls["fromDate"].value
    );
    console.log(
      "this.POOrderList.controls['toDate'].value ==>" +
        this.POOrderList.controls["toDate"].value
    );
    ...
    if (this.POOrderList.controls["Plant"].value) {
      for (var b = 0; b < this.tempPlantList.length; b++) {
        if (
          this.tempPlantList[b].PLANTNAME ==
          this.POOrderList.controls["Plant"].value.trim()
        ) {
    ...
    if (this.POOrderList.controls["fromDate"].value) {
      ...
      this.fromdateofpo = moment(
        new Date(this.POOrderList.controls["fromDate"].value)
      ).format("DD/MM/YYYY");
      this.todateofpo = moment(
        new Date(this.POOrderList.controls["toDate"].value)
      ).format("DD/MM/YYYY");
    } else {
      this.fromdateofpo = "NA";
      this.todateofpo = "NA";
      ...
      this.purchaseOrderListService
        ...
        .subscribe((res1) => {
          if (res1[0].message != "No Data Found for given Vendor Id") {
            ...
            this.endpagenumber = Math.ceil(
              Number(res1[0].popages) / this.pageSize
            );
            if (
              Number(res1[0].popages) / this.popaginationnumber <=
              this.pageSize
            ) {
              this.disablenext = true;
            } else {
  ...
  ResetData() {
    // debugger;
    this.POOrderList.reset();
    this.POOrderList.controls.PONumber.setValue("");
    this.POOrderList.controls.duration.setValue("");
    this.POOrderList.controls.fromDate.setValue("");
    this.POOrderList.controls.toDate.setValue("");
    this.POOrderList.controls.Plant.setValue("");
  ...
  CreateAllDeliveries(val, val1, event, ponumValue, poDate, vendorid) {
    ...
    for (var i = 0; i < this.uniquelineitems.length; i++) {
      if (
        $("#deliveryCheck" + this.selecteddeliveryVal + i).prop("checked") ==
        true
      ) {
        if (
          $("#DcId" + this.selecteddeliveryVal + i).val() == "" ||
          $("#DeliveryQtyId" + this.selecteddeliveryVal + i).val() == "" ||
    ...
    for (var i = 0; i < this.uniquelineitems.length; i++) {
      if (
        ...
        this.deliverydetailsModel.DC = $(
          "#DcId" + this.selecteddeliveryVal + i
        ).val();
        if (this.deliveryarray.length > 0) {
          if (
            this.deliveryarray[this.deliveryarray.length - 1].DC ==
            this.deliverydetailsModel.DC
          ) {
        ...
        this.deliverydetailsModel.DQty = $(
          "#DeliveryQtyId" + this.selecteddeliveryVal + i
        ).val();
        this.deliverydetailsModel.DcDate = $(
          "#DCDateID" + this.selecteddeliveryVal + i
        ).val();
        // this.uniquelineitems[i].RATEPERQTY = this.uniquelineitems[i].RATEPERQTY.replace(/,/g, '');
        this.deliverydetailsModel.AMOUNT =
          parseFloat(this.uniquelineitems[i].RATEPERQTY.replace(/,/g, "")) *
          parseFloat(this.deliverydetailsModel.DQty);

        this.orderdetailsModel.DC = $(
          "#DcId" + this.selecteddeliveryVal + i
        ).val();
        this.orderdetailsModel.DQty = $(
          "#DeliveryQtyId" + this.selecteddeliveryVal + i
        ).val();
        this.orderdetailsModel.DcDate = $(
          "#DCDateID" + this.selecteddeliveryVal + i
        ).val();
        this.orderdetailsModel.AMOUNT =
          parseFloat(this.uniquelineitems[i].RATEPERQTY.replace(/,/g, "")) *
          parseFloat(this.orderdetailsModel.DQty);

        this.orderdetailsModel.LINEITEMNUMBER =
          this.uniquelineitems[i].LINEITEMNUMBER;
        ...
        this.deliverydetailsModel.LINEITEMNUMBER =
          this.uniquelineitems[i].LINEITEMNUMBER;
        this.deliverydetailsModel.lineType = "LineItemDelivery";
        this.deliverydetailsModel.LINEITEMTEXT =
          this.uniquelineitems[i].LINEITEMTEXT;
        this.deliverydetailsModel.UNITOFMEASURE =
          this.uniquelineitems[i].UNITOFMEASURE;
        ...
        this.deliverydetailsModel.RATEPERQTY =
          this.uniquelineitems[i].RATEPERQTY;
        this.deliverydetailsModel.BALANCE_QTY =
          this.uniquelineitems[i].BALANCE_QTY;
        ...
        console.log(
          this.deliverydetailsModel.TOTALAMONT,
          "this.deliverydetailsModel.TOTALAMONT"
        );
    ...
    sessionStorage.setItem(
      "invsubmissionDetails",
      JSON.stringify(this.deliveryarray)
    );
    sessionStorage.removeItem("invsubmissionorderDetails");
    sessionStorage.setItem(
      "invsubmissionorderDetails",
      JSON.stringify(this.orderarray)
    );
    console.log(this.deliveryarray, "this.deliveryarray");
    console.log(
      "this.uniquedeliveryitems create deliver======>>>",
      JSON.stringify(this.uniquedeliveryitems)
    );
  ...
  CreateAllDeliveries1(val, val1, event, ponumValue, vendorid, poDate) {
    ...
    for (var i = 0; i < this.orderlist.length; i++) {
      if (
        $("#deliverydetail" + val + this.lineitemvalue + i).prop("checked") ==
        true
      ) {
    ...
    if (this.selectedLineItem.length == 1) {
      ...
      console.log(
        "this.uniquedeliveryitems create deliver1======>>>",
        JSON.stringify(this.uniquedeliveryitems)
      );
      console.log(
      ...
      for (var i = 0; i < this.uniquedeliveryitems.length; i++) {
        ...
        for (var c = 0; c < this.lineItemOrderarray.length; c++) {
          ...
          console.log(
            "this.orderlist[c].vendorid" + this.orderlist[c].vendorId
          );
          ...
          this.deliverydetailsModel.LINEITEMTEXT =
            this.uniquedeliveryitems[i].LINEITEMTEXT;
          this.deliverydetailsModel.UNITOFMEASURE =
            this.uniquedeliveryitems[i].UNITOFMEASURE;
          this.deliverydetailsModel.Per = 1;
          console.log(
            "this.uniquedeliveryitems[i] ==>" + this.uniquedeliveryitems[i]
          );
          this.deliverydetailsModel.vendorId = vendorid;
          this.deliverydetailsModel.QUANTITY =
            this.uniquedeliveryitems[i].QUANTITY;
          this.deliverydetailsModel.CURRENCY =
            this.uniquedeliveryitems[i].CURRENCY;
          this.deliverydetailsModel.RATEPERQTY =
            this.uniquedeliveryitems[i].RATEPERQTY;
          this.deliverydetailsModel.BALANCE_QTY =
            this.uniquedeliveryitems[i].BALANCE_QTY;
          this.deliverydetailsModel.MATERIAL =
            this.uniquedeliveryitems[i].MATERIAL;
          ...
          if (this.deliveryarray.length == 0) {
            ...
            this.deliveryarray[c] == this.deliverydetailsModel.LINEITEMNUMBER
          ) {
          }
          // }
          // break
      ...
      sessionStorage.setItem(
        "invsubmissionDetails",
        JSON.stringify(this.selectedorderlineitemdetails)
      );
      sessionStorage.removeItem("invallDetails");
      sessionStorage.setItem(
        "invallDetails",
        JSON.stringify(this.lineItemOrderarray)
      );
  ...
  createnewDelivery() {
    ...
    for (var a = 0; a < this.uniquelineitems.length; a++) {
      let deliveryQty = 0;
      this.createDeliverySubmissionsModel =
        new createDeliverySubmissionsModel();
      this.createDeliverySubmissionsModel.businesspartneroid =
        sessionStorage.getItem("Bid");
      this.createDeliverySubmissionsModel.ponumber =
        this.uniquelineitems[a].PO_NUMBER;
      this.createDeliverySubmissionsModel.lineitemnumber =
        this.uniquelineitems[a].LINEITEMNUMBER;
      this.createDeliverySubmissionsModel.lineitemtext =
        this.uniquelineitems[a].LINEITEMTEXT;

      if ((this.isdelieverycheck = true)) {
        this.createDeliverySubmissionsModel.ordernumber = $(
          "#DcId" + this.selecteddeliveryVal + a
        ).val();
        this.createDeliverySubmissionsModel.quantity = $(
          "#DeliveryQtyId" + this.selecteddeliveryVal + a
        ).val();
        this.createDeliverySubmissionsModel.deliverydate = $(
          "#DCDateID" + this.selecteddeliveryVal + a
        ).val();
        console.log(
          this.createDeliverySubmissionsModel.ordernumber,
          "  this.createDeliverySubmissionsModel.ordernumber"
        ...
        this.createDeliverySubmissionsModel.ordernumber = $(
          "#DcId" + this.selecteddeliveryVal + this.deliveryVal1
        ).val();
        this.createDeliverySubmissionsModel.quantity = $(
          "#DeliveryQtyId" + this.selecteddeliveryVal + this.deliveryVal1
        ).val();
        this.createDeliverySubmissionsModel.deliverydate = $(
          "#DCDateID" + this.selecteddeliveryVal + this.deliveryVal1
        ).val();
      }

      this.createDeliverySubmissionsModel.balance_qty =
        this.uniquelineitems[a].BALANCE_QTY;
      this.createDeliverySubmissionsModel.businesspartnertext =
        this.uniquelineitems[a].BUSINESSPARTNERTEXT;
      this.createDeliverySubmissionsModel.deliveryaddress1 =
        this.uniquelineitems[a].DELIVERYADDRESS1;
      this.createDeliverySubmissionsModel.department =
      ...
      this.createDeliverySubmissionsModel.material_type =
        this.uniquelineitems[a].MATERIAL;
      this.createDeliverySubmissionsModel.pincode =
        this.uniquelineitems[a].PINCODE;
      this.createDeliverySubmissionsModel.plant = this.uniquelineitems[a].PLANT;
      this.createDeliverySubmissionsModel.rateperqty =
        this.uniquelineitems[a].RATEPERQTY;
      this.createDeliverySubmissionsModel.state = this.uniquelineitems[a].STATE;
      this.createDeliverySubmissionsModel.unitofmeasure =
        this.uniquelineitems[a].UNITOFMEASURE;
      this.createDeliverySubmissionsModel.remark =
        this.uniquelineitems[a].REMARK;
      this.createDeliverySubmissionsModel.contactpersonemailid =
        sessionStorage.getItem("Requisitioner");
      this.createDeliverySubmissionsModel.contactpersonphone =
        this.uniquelineitems[a].CONTACTPERSONPHONE;
      this.createDeliverySubmissionsModel.costcentre =
        this.uniquelineitems[a].COSTCENTRE;
      this.createDeliverySubmissionsModel.category =
    ...
    this.createDeliveryService
      .newCreateDelivery(this.createDeliveryarray)
      .subscribe((res) => {
  ...
  getOrderDEtails(
    ...
    if (event.target.checked == true) {
      if (this.lineItemOrderarray.length > 0) {
        if (
          this.lineItemOrderarray[this.lineItemOrderarray.length - 1]
            .PO_NUMBER != ponum
        ) {
      ...
      if (event.target.checked == true) {
        ...
        if (this.selectedLineItem.length == 0) {
          ...
          this.selectedLineItem[this.selectedLineItem.length - 1] == lineitem
        ) {
          this.selectedLineItem.push(lineitem);
          this.orderselectedList.push(orderNum);
          this.lineitemorderModel.PO_NUMBER = ponum;
    ...
    for (var i = 0; i < this.orderlist.length; i++) {
      if (
        $("#deliverydetail" + value0 + this.lineitemvalue + i).prop(
          "checked"
        ) == true
      ) {
        this.checkboxisselected = true;
  ...
  navigatetoURL() {
    ...
    if (this.invoicenumber != undefined) {
      this.purchaseOrderListService
        .getwopodetails(this.invoicenumber)
        .subscribe((res) => {
  ...
  navigatetoURL2(ponumber, podate, vendorid) {
    ...
    this.quantitylist = [];
    this.purchaseOrderListService
      .getPODetails(this.fullpoponumber)
      .subscribe((res) => {
        this.poDetail = res[0].poData;
  ...
  submitwholePOTest() {
    console.log(
      "Am I getring ===========>",
      sessionStorage.getItem("poNumber")
    );
    if (this.invoicenumber != undefined) {
      this.purchaseOrderListService
        .getwopodetails(this.invoicenumber)
  ...
  submitwholePO() {
    ...
    this.purchaseOrderListService
      ...
      .subscribe((res) => {
        if (res[0].message == "Success") {
          this.purchaseOrderListService
            .getorderforfullpo(this.fullpoponumber)
            .subscribe((res2) => {
              if (res2[0].message == "Success") {
                ...
                for (let c = 0; c < this.poorders.length; c++) {
                  ...
                  this.orderdetailsModel.LINEITEMNUMBER =
                    this.poorders[c].LINEITEMNUMBER;
                  ...
                  this.lineitemorderModel.LINEITEMNUMBER =
                    this.poorders[c].LINEITEMNUMBER;
                  ...
                  for (let x = 0; x < this.poDetail.length; x++) {
                    console.log(
                      "this.poDetail[x].LINEITEMNUMBER" +
                        this.poDetail[x].LINEITEMNUMBER
                    );
                    console.log(
                      "this.poorders[c].LINEITEMNUMBER " +
                    ...
                    if (
                      ...
                      console.log(
                        "in here" +
                          this.poDetail[x].RATEPERQTY.replace(/,/g, "")
                      );
                      var a = this.poorders[c].QUANTITY;
                      var b = this.poDetail[x].RATEPERQTY.replace(/,/g, "");
                      this.temp =
                        parseFloat(this.poorders[c].QUANTITY) *
                        parseFloat(this.poDetail[x].RATEperqty);
                      console.log("temp is here " + this.temp);
                    }
                    this.lineitemorderModel.DcDate =
                      this.poorders[c].DISPATCHDATE;
                  }
                  var total = a * b;
                  this.lineitemorderModel.AMOUNT = total;
                  console.log(
                    "this.lineitemorderModel.AMOUNT ==> " +
                      this.lineitemorderModel.AMOUNT
                  );
                  this.lineItemOrderarray.push(this.lineitemorderModel);
                }
                console.log(
                  "this.lineItemOrderarray " + this.lineItemOrderarray.length
                );
  ...
  checklength(type) {
    // console.log("in", $('#messagetext').val().length);
    if (type == "remark") {
      let count_elements = $("#messagetext").val().length;
  ...
  getPlantData(e) {
    ...
    this.trackOrderListService
      .getPlanCode(e.target.value.toUpperCase())
      .subscribe((res) => {
  ...
  invoiceMultiplePo(ponumber, date, vendorid, type, poType, plantType) {
    sessionStorage.removeItem("invwopodetails");
